...
with tab3:
    ...
    buyer = st.text_input(T["buyer_label"])
    if st.button(T["ask_button"]) and buyer:
...
with tab6:
    ...
    if df.empty:
        ...
        # Personalize using this farmer's own last sale for this crop, if any —
        # everything here is pure Python (db lookup + market.py arithmetic);
        # the AI below only phrases it, same rule as the rest of this app.
        last_sale = get_last_sale_for_crop(FARMER_ID, crop)
        personal_insight = compute_personal_insight(alert, last_sale)

        if personal_insight:
            move_label = T.get("higher", "higher") if personal_insight["better_now"] else T.get("lower", "lower")
            days_ago = personal_insight["days_ago"]
            when_bit = (
                f"{days_ago} {T.get('days_ago', 'days ago')}"
                if days_ago is not None
                else T.get("recently", "recently")
            )
            st.caption(
                f"📋 {T.get('your_last_sale', 'Your last sale')}: "
                f"₹{personal_insight['last_price']}/quintal "
                f"({personal_insight.get('last_buyer') or T.get('a_buyer', 'a buyer')}, {when_bit}) — "
                f"{T.get('today_is', "today's price is")} "
                f"{abs(personal_insight['pct_vs_last_sale'])}% {move_label}."
            )

        # Gemini only phrases the suggestion — all numbers above are pure Python math
        advice = generate_market_advice(alert, crop, language_hint=GEMINI_LANG, personal_insight=personal_insight)
